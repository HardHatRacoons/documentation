...
System testing will be done at the end of the project, when we integrate our front-end and back-end together. We simulate end-to-end user interactions with the application through a series of tasks. We verify the correctness of the authentication, navigation and display flow that a user may follow when using the app, including errors they may encounter.
...
Several opportunities exist to enhance and expand the current system, either by addressing features that were beyond the scope of this project or by introducing additional capabilities aligned with sponsor and user feedback.

One avenue for improvement is the expansion of data visualizations. Future teams could implement additional graphs and metrics to provide a more detailed breakdown of the blueprint data, such as page-by-page analysis or material summaries by floor. Enhancements such as PDF and graph zooming functionality would also improve user experience by making information more accessible and easier to interpret. Another potential enhancement involves implementing annotation-hover functionality within the blueprint viewer. Allowing users to quickly link visual blueprint elements to tabular data would create a more integrated and intuitive platform. While this feature was deprioritized during the current project due to time constraints, it remains a valuable addition for improving user interaction.

Further optimization of the backend clustering algorithm represents another area for development. Reducing runtime and improving the scalability of the algorithm, particularly for large or complex blueprints, would further strengthen system performance. Additionally, more seamless integration between the clustering and annotation components could streamline the overall processing pipeline.

Looking ahead, future teams may also explore generating 3D models from extracted beam data. Providing a 3D visualization of the blueprint could significantly enhance user comprehension and project planning, and could be integrated as an optional view within the current web application.

Finally, improvements to the file management system such as enabling folder-based organization, multi-file uploads, and batch operations would further enhance usability, particularly for clients managing large volumes of blueprints. Collectively, these future directions offer significant potential to build upon the foundation laid by the current project, driving greater efficiency, accessibility, and user satisfaction.

## 10 Conclusion

This capstone project successfully delivers an automated system for construction blueprint analysis, combining backend algorithmic processing with an accessible and responsive frontend web application. Through integrating technologies such as Python, ReactJS, AWS Amplify, and Elastic Beanstalk, the system enables users to efficiently upload vector-format PDFs and retrieve annotated documents and material cost breakdowns, significantly reducing the manual labor traditionally required.

Rigorous user studies confirmed the platform’s effectiveness, with participants consistently praising the ease of navigation, clarity of annotations, and responsiveness of the system. Performance benchmarks also validated the algorithm’s ability to process complex blueprints in under 15 minutes (under 3 minutes for sarasota), meeting key project goals for speed and scalability.
